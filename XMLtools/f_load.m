function [ o_1, o_2, o_3, o_4 ] = f_load( i_full_fname, i_format)
...
switch i_format
    ...
    case '3412'
        ...
        for i=1:floor(t_end/4)
            o_1(:,:,i)=double(imread(i_full_fname,4*(i-1)+3));
            o_2(:,:,i)=double(imread(i_full_fname,4*(i-1)+4));
            o_3(:,:,i)=double(imread(i_full_fname,4*(i-1)+1));
            o_4(:,:,i)=double(imread(i_full_fname,4*(i-1)+2));
        end
